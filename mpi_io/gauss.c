...
MPI_Request request,request1,request2;
  ...
  MPI_File_write_at(fh,0,&rows,1,MPI_INT,&status);
  MPI_File_write_at(fh,rank*local_block_size*sizeof(double) + 1*sizeof(int),solution_local_block,local_block_size,MPI_DOUBLE,&status);
  MPI_File_close(&fh);
